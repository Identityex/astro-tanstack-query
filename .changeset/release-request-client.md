...
- A JSON endpoint, a redirect, a page whose visitor disconnected mid-stream, and every response in
  ...
  cancellation. `injectState`'s writer may be `null` to release without emitting, which is what the
  middleware now does in component mode.
...
- A response that is compressed, or declares a charset other than UTF-8, is returned untouched
  instead of being decoded and corrupted; development warns once and points to
  `emit: "component"`. A UTF-8 byte order mark is kept.
