...
`htmx.org` 2.0.10 · `devalue` (current) · Node 24.
...
4. `get()` on an unmounted store mounts it (_verified_: nanostores `get()`
   ...
   (§3.2(4)), so both sides render them as pending and loading, not as an
   error. The shared cache adds one case no snapshot can match: an island
   that hydrates after another island or a script has already fetched an
   un-prefetched query reads data where the server rendered pending.
...
6. Server islands: Astro inserts a `server:defer` response with a single
   ...
   every `.astro-tq` element. For each, it drops the class, checks
   `data-serializer` (a mismatch throws, as in §9) and calls `hydrate()`.
...
4. Response: `injectState(response, client, writer, { warn, island })`. If
   ...
   cancels the reader and `flush` never runs. Any other response is
   cleared at once and returned as the same object; a compressed or
   legacy-charset page also gets one dev warning pointing to
   `emit: 'component'`. In `emit: 'component'` the middleware passes a
   `null` writer: the page goes through a byte-identity stream that clears
   only at flush or cancel, so `<QueryState />` and its siblings can still
   read the client while they render.
...
5. No `AsyncLocalStorage` (exotic runtime): stores return a pending,
   ...
   first read, which fetches because nothing was dehydrated),
   `locals.queryClient` still works, the blob still emits, one
   `console.warn` in dev. ACCEPTED: Cloudflare and Vercel Edge lose ALS
...
`actionMutation` exposes the Action's `ActionError<TInput>`; guard with
`error && isInputError(error)` (TanStack's no-error value is `null`, which
`isInputError`'s typed overload rejects) to read schema-typed `fields`.
`actionQuery` exposes `ActionError | DefaultError`; narrow with
`isActionError()` before using `code` or `status`, and do not expect
schema-typed `fields` there. `actionQueryOptions` is the options form, and
`ActionErrorOf<A>` names an Action's error type. The browser wrappers call
...
does from Astro's context. The plugin fixes the virtual config's
`browserBuild` to `false`, so a DOM-environment test file, which Vitest
loads as client code, still decides its side by `window` at run time.
...
- **Only prefetched, successful queries are dehydrated.** `dehydrate`'s
  ...
  runs once every response has ended, in both emit modes: a page at stream
  flush or cancel (after `<QueryState />` has emitted, in component mode), an
  endpoint or redirect as soon as it returns.
...
`error` (the Action's `ActionError<TInput>` for Action mutations;
`ActionError | DefaultError` for Action queries); an unavailable ALS degrades
...
| A compressed or non-UTF-8 HTML response reaches the transform (e.g. a compressing middleware inside `order: 'pre'`)                      | ACCEPTED | returned untouched without state, so islands fetch in the browser; one dev warning; `emit: 'component'` renders the state into the page before compression                                         |
